...
_enemies::_enemies()
{
    ...
    plySpd = 0.01;                  //The value of _Speed2 when the Player is Walking
...
void _enemies::enmsInit(char ID, float xFrm, float yFrm)
{
    ...
    switch(id)
    {
        ...
        case 'o':                   //Ogre enemy, xFrames should be 9.0, yFrames should be 4.0
            ...
            enemyRadius = 0.4;
            detectRadius = 7.0;
            atkRadius = 0.91;       //AtkRadius for Ogre needs to be bigger than PlayerRadius + EnemyRadius

            enemyHealth = 5 * levelCount;
            enmySpd = 0.008;
            enmyWlkSpd = enmySpd/2.0;
            sizeE.x = 0.8;
            sizeE.y = 0.8;
            ...
            switch(id)                          //Deaf so that it enemies already aggro on player won't be affected by shout
            {
                ...
                case 'p':
                    if((sqrt(powf(posE.x, 2.0) + powf(posE.y, 2.0)) <= atkRadius)){                                            //Will start to chomp if within attack radius
                        if(posE.x < 0.0075 && posE.x > -0.0075){
                            xSpeed = 0.0;
                        }
                        else if(posE.x >= 0.0075){                    //If Enemy is to the Right of the Player
                            if(eT->getTicks() > 120){           //Animation for going Left
                                xMin += 1.0/xFrames;
                                xMax += 1.0/xFrames;
                                yMin = 3.0/yFrames;
                        ...
                        else if(posE.x < -0.0075){              //Else Enemy is to the Left of the Player
                            if(eT->getTicks() > 120){          //Animation for going Right
                                xMin += 1.0/xFrames;
                                xMax += 1.0/xFrames;
                                yMin = 3.0/yFrames;
                        ...
                        if(posE.y < 0.0075 && posE.y > -0.0075){
                            ySpeed = 0.0;
                        }
                        else if(posE.y >= 0.0075){                   //If Enemy is Above the Player
                            if(eT->getTicks() > 120){          //Animation for going Down
                                xMin += 1.0/xFrames;
                                xMax += 1.0/xFrames;
                                yMin = 3.0/yFrames;
                        ...
                        else if(posE.y < -0.0075){              //If Enemy is Below the Player
