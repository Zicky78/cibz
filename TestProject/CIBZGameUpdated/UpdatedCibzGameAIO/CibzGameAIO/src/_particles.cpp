...
void _particles::projectileScroll(float speed, pts part[])
{
    if(!isAttacking && !isBlocking){
        if(lFlag){
            //rFlag = false;
            if(lFlag && isDodging){
                part->xProjDest += speed * 20.0;        //200.0;
                part->xDir += speed * 20.0;             //200.0;
                isDodging = false;
            }
            else{
            ...
            if(lFlag && uFlag && isDodging){
                part->yProjDest -= speed * 20.0;        //200.0;
                part->yDir -= speed * 20.0;             //200.0;
                isDodging = false;
            }
...
void _particles::scroll(float speed, pts part[])
{
    if(!isAttacking && !isBlocking){
        if(lFlag){
            ...
            if(lFlag && uFlag && isDodging){
                part->posY -= speed * 20.0;        //200.0;
                part->movedFloor -= speed * 20.0;        //200.0;
                isDodging = false;
            }
            else if(lFlag && dFlag && isDodging){
                part->posY += speed * 20.0;        //200.0;
...
void _particles::updateProjectile(_player* ply, _enemies enmy[], int numEnmy, int &sumnNum, bool level5)
{
    for(int i = 0; i < numProjDrops; i++){
        if(projectile[i].alive){
