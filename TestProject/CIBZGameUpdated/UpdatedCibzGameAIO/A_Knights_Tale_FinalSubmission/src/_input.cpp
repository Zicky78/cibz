...
void _input::menuNav(_sounds *snds, _menu *menu)
{
    ...
    case 0x36: //if user presses 6, start level 6 (Win)
        ...
        menu->doneLoading = false; //false
        snds->playMusic("sounds/gamewin.mp3");
        menu->currLevel = 6;
    break;
...
void _input::keyUp(_menu* menu)
{
    ...
    case 0x36: //if user presses 6, start level 6
...
void _input::moveEnv(_parallax* plx, _player* ply, _enemies enem[], _items* item, int numOfEnem, float speed, _door* door, _particles * part)
{
        ...
        if(!ply->isAttacking && !ply->isBlocking){
            if(tmr->getTicks() > 2400){
                ...
                if(!ply->dodgingLimit){
                    ...
                    //Sounds
                    snds->playSounds("sounds/dodge.mp3");
                }
    ...
    case VK_SHIFT:
        if(!ply->isBlocking){
            ...
            part->isAttacking = true;
             ...
             //Sounds
            if(sndTmr->getTicks() > 100){
                snds->playSounds("sounds/swordhit2.mp3");
                sndTmr->resetTime();
            }
    ...
    case VK_CAPITAL:
        if(!ply->isBlocking){
            ...
            //Sounds
            if(sndTmr->getTicks() > 100){
                snds->playSounds("sounds/swordheavy.mp3");
                sndTmr->resetTime();
            }
    ...
    case 67:                            //C key for Blocking
        ...
        if(sndTmr->getTicks() > 100){
            snds->playSounds("sounds/block.mp3");
            sndTmr->resetTime();
        }
        break;
    }
...
void _input::mouseDown(_player* ply, _parallax* plx, float x, float y, _items* item, _enemies* enem, int numOfEnem,  _door* door, _particles* part, _menu* menu)
{
    ...
    case MK_LBUTTON:
        if(!ply->isBlocking){
            ...
            //Sounds
            if(menu->liveLevel1 || menu->liveLevel2 || menu->liveLevel3 || menu->liveLevel4 || menu->liveLevel5){
            snds->playSounds("sounds/swordhit2.mp3");
            }
